...
패키지 대부분은 작은 **규칙 엔진**이다. *팩(pack)* 하나가 pi 훅, 보낼 state, Jev에 물을 질문,
...
```
  ...
  graph              turn_end                     어느 단계? 진전? 이탈?             HUD + 불변식
...
**state 소스.** `cwd`, `user_request`, `last_user_message`, `recent_turns`(최근 6턴, 각 600자,
시스템 프롬프트 제외), `tool`, `arguments`, `output`(앞 `outputChars`자), `is_error`, `prompt`
(before_agent_start), `assistant_text`, `turn_tools` / `run_tools`(툴 요약: 이름, 짧은 인자,
에러 여부, 출력 앞 300자), `recent_failures`(브랜치에서 실패한 최근 5회), `turn_index`,
`graph`(작업 그래프 스냅샷), `evidence`(현재 프롬프트 장부: 상태, 바뀐 파일, 마지막 검증,
페이즈, 불변식, 마지막 최종 메시지).
...
비교는 거짓이다. 코드를 호출할 방법은 없다. 답 외에 스코프에는 `vars`, `tool`,
`is_error`, `redactions`(로컬에서 지운 자격증명 수), `turn_index`, 툴 궤적의 횟수
(`run_edits`, `run_errors`, `run_tool_count`, `turn_edits`, `turn_errors`), 장부 사실
(`files_changed`, `verifications`, `verified_after_change`)이 있다.
...
`block`, `confirm`, `steer`, `set_thinking`, `set_tools`는 *개입*이다. shadow에서는
"would block …"으로만 보고되고 `shadow:block`으로 기록되며 그 외에는 아무 일도 없다.
`annotate`, `warn`, `status`, `tag`는 두 모드 모두 실행된다.
...
4. 설정 `packs.<name>`에 `on`과 `questions`가 있는 객체
...
**gate** — `bash`, `write`, `edit`의 `tool_call`. 한 요청에 질문 넷: `destructive`,
...
**output** — `bash`의 `tool_result`. `leaks_secret` (noul)과 `failure_class` (choice:
...
종류마다 조언 한 줄 (transient → 그대로 한 번 재시도, environment → 코드가 아니라
머신을 고침, permission → 사용자에게 물음). 절대 막지 않는다.

**intent** — `before_agent_start`. `intent` (choice: question, small_edit, feature, debug,
...
**honest_finish** — `agent_end`. 최종 메시지와 런의 툴 궤적에 대해 `claims_done`, `verified`,
...
**stuck** — `turn_end`. 세 번째 턴부터, 그 턴에 실패한 호출이 있을 때만.
...
세고, 에디터 아래에 한 줄 HUD를 그리며, 불변식 셋을 코드로 검사한다.
...
`/quiet graph`는 경로, 턴별 표, 전이 횟수를 출력한다. 기본은 shadow (HUD + 히스토리만).
`/quiet graph enforce`는 불변식당 프롬프트당 한 번 조향한다. 팩은 `"state": ["graph", …]`로
그래프를 읽을 수 있다.
...
키 탐색: `TYPESAFE_API_KEY` 환경변수 → `apiKey` → `apiKeyFile` → `<cwd>/.env`.
키는 알림, 히스토리 줄, 조향 메시지에서 지워진다.
...
`…:outcome`으로 덧붙는다. 각 레코드에는 `kind`(팩 이름), `action`, `mode`, `summary`,
원본 `answers`, `latencyMs`, `cached`, 실제로 보낸(마스킹된) `state`가 있다.
...
팩이 나열한 state만 나간다. 그 전에 알려진 자격증명 형태는 `<REDACTED:kind>`로 바뀌고
`redactions`에 세며, 문자열은 `argumentChars` / `outputChars`, 전체는 `maxStateChars`에서
자른다. 요청은 `timeoutMs`에 재시도 없이 타임아웃한다. 타임아웃, 4xx/5xx, 잘못된 응답은
*판정 없음*이고, pi는 원래대로 동작한다. 오류는 분당 한 번만 알린다. 같은 요청은 in-flight를
공유하고, 팩이 `cacheSeconds`를 두면 답도 한 번만 받는다.
...
여기서 새로운 것: 선언적 규칙 엔진과 사용자 팩, `before_agent_start` / `turn_end` /
`agent_end` 판정기(intent, honest_finish, stuck), 작업 그래프, pi-ask를 대체하지 않는 선별,
